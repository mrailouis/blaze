...
object WorldBoxRenderer {
    private val pipeline: RenderPipeline = RenderPipelines.register(
        RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation(BlazeIdentifiers.of("blaze", "pipeline/world_boxes"))
            .build()
    )
    private val allocator = ByteBufferBuilder(786_432)
    ...
    fun render(
        ...
        for (quad in quads) {
            renderFilledQuad(pose, builder, quad)
        }
    ...
    private fun ensureBuffer(): BufferBuilder {
        ...
        return BufferBuilder(allocator, pipeline.vertexFormatMode, pipeline.vertexFormat).also { buffer = it }
    }
    ...
    private fun renderFilledQuad(positionMatrix: Matrix4f, buffer: BufferBuilder, quad: SurfaceQuad) {
        val style = quad.style
        val points = listOf(quad.a, quad.b, quad.c, quad.d)
        for (point in points) {
            buffer.addVertex(positionMatrix, point.x.toFloat(), point.y.toFloat(), point.z.toFloat())
                .setColor(style.fillRed, style.fillGreen, style.fillBlue, style.fillAlpha)
        }
        for (point in listOf(quad.d, quad.c, quad.b, quad.a)) {
            buffer.addVertex(positionMatrix, point.x.toFloat(), point.y.toFloat(), point.z.toFloat())
                .setColor(style.fillRed, style.fillGreen, style.fillBlue, style.fillAlpha)
        }
    }
    ...
    private fun draw(
        ...
        val renderPass = if (depthless) {
            encoder.createRenderPass(
                { "blaze world box render pass depthless" },
                colorView,
                OptionalInt.empty()
            )
        } else {
