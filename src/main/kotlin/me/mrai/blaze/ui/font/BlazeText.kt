...
object BlazeText {
    private const val supersample = 6f
    ...
    fun prewarmClickGui() {
        ...
        warmStrings.forEach { text ->
            cache.computeIfAbsent(TextKey(text, 0xFFFFFFFF.toInt(), 0xFFFFFFFF.toInt(), FontKind.BODY), ::bake)
            cache.computeIfAbsent(TextKey(text, 0xFFFFFFFF.toInt(), 0xFFFFFFFF.toInt(), FontKind.LABEL), ::bake)
        }
        cache.computeIfAbsent(TextKey("BLAZE", 0xFFFF4E2F.toInt(), 0xFFFFD54A.toInt(), FontKind.TITLE), ::bake)
    }
    ...
    fun widthScaled(font: Font, text: String, scale: Float, bold: Boolean = false): Int {
        ...
        return (cached(text, kind).drawWidth * kind.renderScale * scale).roundToInt()
    }

    fun heightScaled(font: Font, text: String, scale: Float, bold: Boolean = false): Int {
        val kind = if (bold) FontKind.LABEL else FontKind.BODY
        return (cached(text, kind).drawHeight * kind.renderScale * scale).roundToInt()
    }

    fun centerOffset(font: Font, text: String, bold: Boolean = false): Float {
        val kind = if (bold) FontKind.LABEL else FontKind.BODY
        val cached = cached(text, kind)
        return (((cached.contentTop + cached.contentBottom + 1) / 2f) / supersample) * kind.renderScale
    }

    fun titleWidth(text: String, scale: Float): Int {
        return (cached(text, FontKind.TITLE).drawWidth * FontKind.TITLE.renderScale * scale).roundToInt()
    }
    ...
    private fun drawInternal(
        ...
        val cached = cache.computeIfAbsent(TextKey(text, startColor, endColor, kind), ::bake)
        val pose = graphics.pose()
        pose.pushMatrix()
        pose.translate(x.toFloat(), y.toFloat())
        pose.scale((scale * kind.renderScale) / supersample, (scale * kind.renderScale) / supersample)
        graphics.blit(
    ...
    private fun bake(key: TextKey): CachedText {
        ...
        val extraPassWidth = when (key.kind) {
            FontKind.BODY -> 0
            FontKind.LABEL -> 1
            FontKind.TITLE -> 2
        }
        val bakedWidth = ((textWidth + texturePadding * 2 + extraPassWidth) * supersample).toInt().coerceAtLeast(1)
        val bakedHeight = ((textHeight + texturePadding * 2) * supersample).toInt().coerceAtLeast(1)

        val image = BufferedImage(bakedWidth, bakedHeight, BufferedImage.TYPE_INT_ARGB)
        val graphics = image.createGraphics()
        applyHints(graphics)
        graphics.transform = AffineTransform.getScaleInstance(supersample.toDouble(), supersample.toDouble())
        ...
        drawStyledString(graphics, key.text, texturePadding.toFloat(), (texturePadding + ascent).toFloat(), key.kind)
        graphics.dispose()
        ...
        val id = Identifier.fromNamespaceAndPath("blaze", "generated/text_$index")
        ...
        return CachedText(id, bakedWidth, bakedHeight, textWidth + extraPassWidth, textHeight, contentTop, contentBottom)
    }

    private fun cached(text: String, kind: FontKind): CachedText {
        return cache.computeIfAbsent(TextKey(text, 0xFFFFFFFF.toInt(), 0xFFFFFFFF.toInt(), kind), ::bake)
    }

    private fun drawStyledString(
        ...
        when (kind) {
            FontKind.BODY -> Unit
            FontKind.LABEL -> graphics.drawString(text, x + 0.20f, baseline)
            FontKind.TITLE -> {
                graphics.drawString(text, x + 0.30f, baseline)
                graphics.drawString(text, x + 0.60f, baseline)
            }
        }
    }
    ...
    private fun fontFor(kind: FontKind): AwtFont {
        return fontCache.computeIfAbsent(kind) { selected ->
            loadFont(selected.size, selected.style).deriveFont(
                AffineTransform.getScaleInstance(selected.xScale.toDouble(), 1.0)
            )
        }
    }
    ...
    private enum class FontKind(
        ...
        BODY(12f, 1.0f, AwtFont.PLAIN, 0.75f),
        LABEL(12f, 1.0f, AwtFont.BOLD, 0.75f),
        TITLE(16f, 0.90f, AwtFont.BOLD, 0.82f)
    }
